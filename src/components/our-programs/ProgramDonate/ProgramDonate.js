...
export default function ProgramDonate ({ program: initialProgram = null }) {
  ...
  const [program, setProgram] = useState(
    initialProgram ? initialProgram.slug : ''
  )
  ...
  // Update wallet address and network when selected method changes (crypto only)
  useEffect(() => {
    if (selectedMethod === 'BANK') {
      setWalletAddress('') // No wallet address for bank
      // Network not needed for bank
    } else {
      setWalletAddress(cryptoWallets[selectedMethod] || '')
      const asset = paymentMethods.find(
        m => m.symbol === selectedMethod && m.type === 'crypto'
      )
